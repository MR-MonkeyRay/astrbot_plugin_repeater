...
class RepeaterPlugin(Star):
    ...
    async def _handle_group_message(self, event: AstrMessageEvent) -> None:
        ...
        if group_key not in self.group_states and not self.default_enabled:
            return
        ...
        logger.info(f"[repeater] {group_key} 触发复读: {text[:20]}")
    ...
    async def _process_message(
        ...
        async with self.save_lock:
            ...
            should_repeat = (
                len(state.repeated_users) >= self.repeat_threshold
